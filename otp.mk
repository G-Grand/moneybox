...
VM       := etc/$(shell hostname)/vm.args
SYS      := etc/$(shell hostname)/sys.config
PLT_NAME := ~/.n2o_dialyzer.plt
ERL_ARGS := -args_file $(VM) -config $(SYS)
RUN_DIR  := apps/moneybox/priv/static/log
LOG_DIR  := apps/moneybox/priv/static/log/log
empty    :=
ROOTS    := apps deps
space    := $(empty) $(empty)
comma    := $(empty),$(empty)
...
stop:
	@kill -9 $(shell ps ax -o pid= -o command=|grep beam|grep `hostname`|awk '{print $$1}')
$(PLT_NAME):
